...
def calculate_baseline_metrics(logs_path="logs.csv"):
    ...
    try:
        ...
        total_duration_days = (
            df["EndTime"].max() - df["StartTime"].min()
...
def evaluate():
    ...
    print(
        f"Starting evaluation for {num_episodes} long episode(s) of {sim_duration} minutes..."
    )
    ...
    for episode in range(num_episodes):
        ...
        for c in episode_completed_cases:
            print(
                f"DEBUG: Case {c.id} Status: {c.status}, Duration: {c.completion_time - c.creation_time if c.completion_time else 'N/A'}"
            )
    ...
    print("-" * 60)
    print(
        f"{'Avg Case Duration (min)':<25} | {baseline['avg_duration'] if baseline else 'N/A':<15.2f} | {rl_avg_duration:<15.2f}"
    )
    print(
        f"{'Throughput (cases/day)':<25} | {baseline['throughput_per_day'] if baseline else 'N/A':<15.2f} | {rl_throughput:<15.2f}"
    )
    print(
        f"{'Avg Queue Length':<25} | {'N/A':<15} | {np.mean(all_queue_lengths):<15.2f}"
    )
    print(
        f"{'Avg Utilization':<25} | {'N/A':<15} | {np.mean(all_worker_utilization):<15.2f}"
    )
    print("=" * 40)

    if baseline and rl_avg_duration > 0:
        improvement = (
            (baseline["avg_duration"] - rl_avg_duration)
