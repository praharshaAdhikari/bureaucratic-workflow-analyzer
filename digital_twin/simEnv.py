...
class SimEnv(gym.Env):
    ...
    def _define_spaces(self) -> None:
        ...
        # Actions: 0=do nothing, 1=reassign, 2=increase priority, 3=add worker
    ...
    def _apply_action(self, action: int) -> None:
        ...
        Actions:
        0: Do nothing (important baseline - not all states need intervention)
        1: Reassign tasks to less loaded workers
        2: Increase priority of oldest pending case
        3: Add temporary worker (expensive, use sparingly)
        """
        if action == 0:
            # Do nothing - this is a valid choice!
            pass
        elif action == 1:
            self._reassign_task()
        elif action == 2:
    ...
    def _add_temporary_worker(self) -> None:
        ...
        # Reduce max from 5 to 3 to encourage efficiency
        if len(temp_workers) >= 3:
            print("Max temporary workers reached (3). Cannot add more.")
            return
