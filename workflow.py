...
class Workflow:
    ...
    def read_logs(self, filename: str = 'logs.csv') -> pd.DataFrame:
        """
        ...
        df['Duration'] = (df['EndTime'] - df['StartTime']).dt.total_seconds() / 60.0

        self.logs = df.sort_values(by='StartTime')
        return df
    ...
    def read_ideal_durations(self, filename: str = 'ideal_durations.csv') -> pd.DataFrame:
        """
        ...
        if not os.path.exists(filename):
            print(f"{filename} not found. Running prompt_and_save_ideal_durations() first.")
    ...
    def start(self, speed: float) -> None:
        ...
        for case_id in self.logs['CaseID'].unique():
            ...
            # Add all tasks to the case first
            for _, task_row in tasks.iterrows():
                task = Task(task_row['Task'], task_row['Duration'])
                case_.tasks.append(task)
                worker = next((w for w in self.workers if w.name == task_row['Worker']), None)
                if worker:
            ...
            for i, (_, task_row) in enumerate(tasks.iterrows()):
                ...
                if worker:
                    self.env.process(case_.assign_task(worker, task, speed, task_row['Status']))

    def generate_report(self, filename: str = 'workflow_report.txt') -> None:
        ...
        if self.ideal_task_durations is None:
            print("Ideal task durations not loaded. Please run read_ideal_durations() first.")
            return
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            ...
            write_both(f"Accepted Cases: {accepted_cases} ({accepted_cases/total_cases*100:.1f}%)")
            write_both(f"Rejected Cases: {rejected_cases} ({rejected_cases/total_cases*100:.1f}%)")
