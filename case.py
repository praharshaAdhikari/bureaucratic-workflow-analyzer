...
class Case:
    ...
    def assign_task(self, worker: Worker, task: Task, speed: float = 1.0, expected_status: str = 'Completed') -> Generator:
        """
        ...
        yield self.env.process(worker.perform_task(task, self.env, speed, expected_status))
    ...
    def check_completion(self) -> None:
        ...
        if all(task.status != 'Pending' for task in self.tasks):
            ...
            self.status = 'Accepted' if all(task.status == 'Completed' for task in self.tasks) else 'Rejected'
            print(f"Case {self.id} completed with status: {self.status} at time {self.env.now}")
